    ...
    <div class="content" v-if="getShoppingCart.length>0">
      <ul>
        ...
        <li class="product-list" v-for="(item,index) in getShoppingCart" :key="item.productId">
          ...
          <div class="pro-price">{{item.promotionPrice}}元</div>
          <div class="pro-num">
            <el-input-number
              size="small"
              :value="item.count"
          ...
          <div class="pro-total pro-total-in">{{item.promotionPrice*item.count}}元</div>
...
import {deleteGoods, updateGoodsCount} from "../api/shopCar";
