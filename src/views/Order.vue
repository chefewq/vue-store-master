    ...
    <div class="order-content" v-if="orders.length>0">
      <div class="content" v-for="(item,index) in orders" :key="index">
        <ul>
          ...
          <!-- 订单列表 -->
          <li class="product-list" v-for="(product,i) in item.orderElements" :key="i">
            <div class="pro-img">
              <router-link :to="{ path: '/goods/details', query: {productID:product.productId} }">
                <img :src="$target + product.imgAddr" />
              </router-link>
            </div>
            <div class="pro-name">
              <router-link
            ...
            <div class="pro-price">{{product.promotionPrice}}元</div>
            <div class="pro-num">{{product.count}}</div>
            <div class="pro-total pro-total-in">{{product.promotionPrice*product.count}}元</div>
          </li>
        </ul>
        <div class="order-bar">
...
<script>
import {selectAllOrder} from "../api/order";
  ...
  watch: {
    ...
    orders: function(val) {
      ...
      for (let i = 0; i < val.length; i++) {
        const element = val[i].orderElements;
        ...
        for (let j = 0; j < element.length; j++) {
          ...
          totalNum += temp.count;
          totalPrice += temp.promotionPrice * temp.count;
        }
