    ...
    <ul>
      <li v-for="item in list" :key="item.productId">
        ...
        <router-link :to="{ path: '/goods/details', query: {productID:item.productId} }">
          <img :src="$target +item.imgAddr" alt />
          <h2>{{item.productName}}</h2>
          <h3>{{item.productTitle}}</h3>
          <p>
            <span>{{item.promotionPrice}}元</span>
            <span
              v-show="item.promotionPrice != item.normalPrice"
              class="del"
            >{{item.normalPrice}}元</span>
          </p>
        </router-link>
      </li>
...
<script>
import {deleteGoodsCollect} from "../api/goodsCollect";
